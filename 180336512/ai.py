...
class Agent:
    ...
    def make_one_transition(self, action):
        if (self.simulator.game_over()):
            return self.simulator.state,None;
        elif (action == HIT):
    ...
    def MC_run(self, num_simulation, tester=False):
        ...
        for simulation in range(num_simulation):
            ...
            ##print((self.simulator.check_reward(),self.simulator.state))
            print((self.simulator.state,self.simulator.check_reward()))
            trajectory = [];
            
            trajectory.append((self.simulator.state,self.simulator.check_reward()))
            while (self.make_one_transition(self.default_policy(self.simulator.state))[1]!= None):
                trajectory.append((self.simulator.state,self.simulator.check_reward()));
           # print(trajectory)
            for i in range(len(trajectory)):
                self.N_MC[trajectory[i][0]] = self.N_MC[trajectory[i][0]] + 1
                for j in range(i,len(trajectory)):
                    self.S_MC[trajectory[i][0]] = self.S_MC[trajectory[i][0]] + pow(DISCOUNT,j-i) * trajectory[j][1]
            for i in trajectory:
                self.MC_values[i[0]] = self.S_MC[i[0]] / self.N_MC[i[0]]
    ...
    def TD_run(self, num_simulation, tester=False):
        ...
        for simulation in range(num_simulation):
            ...
            while (True):
                next_st,rew = self.make_one_transition(self.default_policy(s))
                if (rew == None):
                    self.TD_values[s] = self.TD_values[s] + self.alpha(self.N_TD[s]) * (reward + DISCOUNT * 0 - self.TD_values[s])
                    break;
                self.TD_values[s] = self.TD_values[s] + self.alpha(self.N_TD[s]) * (reward + DISCOUNT * self.TD_values[next_st] - self.TD_values[s])
                reward = rew
                self.N_TD[s] = self.N_TD[s] + 1
                s = next_st
    ...
    def Q_run(self, num_simulation, tester=False, epsilon=0.4):
        ...
        for simulation in range(num_simulation):
            ...
            while (True):
                ...
                next_st,rew = self.make_one_transition(a)
                if (rew == None):
                    self.Q_values[s][a] = self.Q_values[s][a] + self.alpha(self.N_Q[s][a]) * (reward + DISCOUNT * 0 - self.Q_values[s][a])
                    break;
                self.Q_values[s][a] = self.Q_values[s][a] + self.alpha(self.N_Q[s][a]) * (reward + DISCOUNT * max(self.Q_values[next_st][HIT] - self.Q_values[s][a],self.Q_values[next_st][STAND] - self.Q_values[s][a]))
                reward = rew
                self.N_Q[s][a] = self.N_Q[s][a] + 1
                s = next_st
            # TODO
            # Note: Do not reset the simulator again in the rest of this simulation
            # Hint: self.simulator.state gives you the current state of the trajectory
